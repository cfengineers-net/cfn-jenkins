...
# pkg-comment file
cat > pkg-comment << EOF
CFEngine 3 Enterprise: An automated suite tools for data centres.
EOF
# pkg-descr file
cat > pkg-descr << EOF
...
for i in bundlestate.cf data_expand.cf data_regextract.cf ensure_line_present_prepend_append.cf file_hash.cf mapdata.cf readcsv.cf readdata.cf string_mustache.cf; do
 cp -f /tmp/xxx/var/cfengine/share/doc/cfengine/examples/${i} /var/cfengine/share/doc/cfengine/examples/${i}
done

# start packaging CFEngine
cd $WORKDIR
/usr/sbin/pkg_create -j -f $WORKDIR/pkg-plist -c $WORKDIR/pkg-comment -d $WORKDIR/pkg-descr -p "/" cfengine-community-${VER}_${REV}.tbz
if [ ! -d $RESDIR ]; then mkdir -p $RESDIR; fi
cp -f $WORKDIR/cfengine-community-${VER}_${REV}.tbz $RESDIR
