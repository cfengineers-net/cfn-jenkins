...
if [ -f $SOURCE_DIR/cfengine-$CORE.tar.gz ]; then
 ...
 ./configure --prefix=/var/cfengine --with-lmdb=/var/cfengine --with-pcre=/var/cfengine --with-openssl=/var/cfengine --enable-shared LDFLAGS=-L/var/cfengine/lib CPPFLAGS=-I/var/cfengine/include && LD_LIBRARY_PATH=/usr/local/lib gmake CC="gcc -R /var/cfengine/lib" && LD_LIBRARY_PATH=/usr/local/lib gmake install && LD_LIBRARY_PATH=/usr/local/lib gmake install DESTDIR=/tmp/xxx
